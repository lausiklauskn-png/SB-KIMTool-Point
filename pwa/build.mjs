// pwa/build.mjs — assembles the standalone, single-file Such-Werkzeug PWA.
//
// Why a build step (and why it is NOT the tool's "Einbau"):
// The CANONICAL source lives in src/modules/{03,04,22}.js. To honour the
// Live-PWA rule "verteilbare Werkzeuge = eine einzige index.html" we package
// a copy-/download-ready single file. This script just concatenates the three
// module sources (unchanged) between a small PWA shell, so the distributable
// can never silently drift from the canonical modules — re-run to refresh.
//
// Using the tool needs NO build: drop the three <script> tags + init() into a
// page (see docs/components/22_such_widget.md). The build only produces the
// stand-alone download artifact pwa/such-werkzeug.html.
//
// Run: npm run build:pwa  (or: node pwa/build.mjs)
...
const mod03 = read("src/modules/03_embedding.js");
const mod04 = read("src/modules/04_match.js");
const mod22 = read("src/modules/22_such_widget.js");

// Guard: the inline technique is only safe because the modules contain no
// "</script>" sequence. Fail loud rather than ship a broken single file.
for (const [name, src] of [["03", mod03], ["04", mod04], ["22", mod22]]) {
  if (src.includes("</script")) {
    throw new Error(`Modul ${name} enthält </script — Inline-Bündelung unsicher, abbrechen.`);
  }
}

// Inline SVG icon as data-URI (no external asset, copy-/offline-tauglich).
const iconSvg =
...
const manifest = {
  ...
  description:
    "Semantische, server-lose Bedeutungs-Suche im Browser (SBKIM Modul 22).",
  lang: "de",
  start_url: ".",
  scope: ".",
  display: "standalone",
  ...
  theme_color: "#10151c",
  icons: [
    { src: iconDataUri, sizes: "any", type: "image/svg+xml", purpose: "any maskable" },
  ],
};
...
const initScript = `
  // Standalone-Init: KEIN Auto-Init im Modul — wir mounten hier, NACH den
  // Abhängigkeiten. Demo-Korpus für die "App"-Suche wird einmalig lokal,
  // server-los eingebettet (Modul 03 lädt das e5-Modell beim ersten Lauf vom
  // CDN — das ist die einzige Netz-Abhängigkeit des semantischen Pfads).
  (function () {
    "use strict";
    var W = window.SbkimSearchWidget;
    if (!W || typeof W.init !== "function") {
      console.error("Such-Werkzeug: Modul 22 nicht geladen.");
      return;
    }
    var DEMO_KORPUS = [
    ...
    W.init({
      ...
      prepareCorpus: async function () {
        var E = window.SbkimEmbedding;
        if (!E || typeof E.embedPassageBatch !== "function") return [];
        try {
          if (typeof E.isReady === "function" && !E.isReady()) await E.init();
          var vecs = await E.embedPassageBatch(DEMO_KORPUS.map(function (d) { return d.text; }));
          return DEMO_KORPUS.map(function (d, i) {
            return { label: d.label, anchorId: d.label, passageVec: vecs[i] };
          });
        } catch (e) {
          console.warn("Demo-Korpus konnte nicht eingebettet werden (Modell offline?):", e);
          return [];
        }
      },
    }).then(function () {
      console.info("Such-Werkzeug v${TOOL_VERSION}: bereit. 🔍-Blase unten rechts.");
    });
  })();
`;
...
<!--
  ...
  GENERIERT von pwa/build.mjs — nicht von Hand bearbeiten, neu bauen: npm run build:pwa
-->
<html lang="de">
<head>
  <meta charset="utf-8" />
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .card {
      border: 1px solid #1d2730; border-radius: 14px; background: #0e141b88;
...
<body>
  <main>
    ...
    <p class="lead">
      ...
      🔍-Blase unten rechts öffnet das Such-Panel.
    </p>
...
const outPath = resolve(root, "pwa/such-werkzeug.html");
writeFileSync(outPath, html, "utf8");
const kb = (html.length / 1024).toFixed(1);
console.log(`pwa/such-werkzeug.html geschrieben (${kb} KB, Module 03+04+22 inline, v${TOOL_VERSION}).`);
