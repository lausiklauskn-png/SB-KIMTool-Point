# BRIEF — Such-Werkzeug (Modul 22): Browser-Lauf, Quittung, optionale Vertiefung
...
2. `PULS.md` (aktueller Stand, oben der 2026-06-21-Eintrag)
3. **diesen Brief**
...
(der zugewiesene Branch kann zurückliegen — diese Sitzung lag 112 Commits zurück, daher
frischer Branch aus `origin/main`). Offene PRs vorher sichten.

---

## 1. Stand **[Pflicht]**
- **Geliefert (diese Sitzung, Branch `claude/modul22-such-werkzeug-aTHX`, Draft-PR):**
  Modul 22 Such-Werkzeug 1:1 aus Sage in `web/tools/sbkim-such-widget.js`; 04 Match auf
  aktuellen Sage-Stand gehoben (hybridMatch/04.D); Smoke `tests/smoke_bau22_such_widget.mjs`
  **148/148** in `npm test`; Profi-Kachel + Markt-Eintrag; „größer ziehen" nicht-invasiv;
  Impressum/Datenschutz §6a (DE+EN); Standalone-PWA `such-werkzeug.html`. `npm test` 95+148
  grün, `npm run verify` 16/16 (keine JS-Fehler mit Widget).
- **Offen / blockiert:** sichtbarer **Browser-Lauf durch Klaus** (Panel sehen, suchen,
  größer ziehen). **Quittung an Sage** (a/b/c) noch nicht in den Briefkasten geschrieben.
...
- **Pflegen:** nach Klaus' Browser-Lauf die „wartet auf Klaus"-Vermerke in `status.json` /
  `werkzeugkiste.json` / `docs/WERKZEUGE.md` auf „browser-bestätigt" heben (nur wenn wirklich
  gesehen — keine Selbst-Grün-Markierung).
...
- **Optional (nur mit Klaus' Freigabe):**
  - **B3-Richter** (sicherheits-/eignungs-bewusst) — Sage baut das; abwarten und dann 1:1 holen.
