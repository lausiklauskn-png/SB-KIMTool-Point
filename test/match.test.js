...
// Cosine-Similarity = Skalarprodukt (beide L2-normalisiert). Erwartet ≈ 0.8485 ≥ 0.80.
...
test("Match: Cross-Knoten-Score ≥ 0.80 (echter semantischer Match, ~0.8485)", () => {
  const score = dot(ours, sage);
  assert.ok(score >= 0.80, `Score ${score} < 0.80 — kein Match`);
  assert.ok(Math.abs(score - 0.8485) < 0.01, `Score ${score} weicht stark von erwartet 0.8485 ab`);
});
